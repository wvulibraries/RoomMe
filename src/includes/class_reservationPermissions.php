...
class reservationPermissions {
  ...
  public function setupUploadForm(){
       try {
          // call engine
          $engine    = EngineAPI::singleton();
          $localvars = localvars::getInstance();
          $validate  = new validate;
          // create upload form
          $form = formBuilder::createForm('createUploadPermissions');
          $form->linkToDatabase( array(
              'table' => 'reservePermissions'
          ));

          // form titles
          $form->insertTitle = "Upload Permissions File";
          ...
          return '{form name="createUploadPermissions" display="form"}';
      } catch (Exception $e) {
          errorHandle::errorMsg($e->getMessage());
      }
  ...
  public function insertRecord($id, $type, $email){
      try {
          ...
          // SQL Results
          $sql = sprintf("INSERT INTO `reservePermissions` (resourceID, resourceType, username) VALUES (?, ?, ?)");
