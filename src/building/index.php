	...
	<?php foreach($buildingRooms as $I=>$room) { ?>
		...
		<li class="whhp">
			<a href="room/?room=<?php print htmlSanitize($room['ID']);?>"><?php if (!is_empty($room['pictureURL'])) {?>
				<spanmg src="<?php print htmlSanitize($room['pictureURL']);?>" class="buildingListPicture" alt="<?php print htmlSanitize($room['displayName']); ?>" /><span class="reservationsBuildingRoomsListName">
